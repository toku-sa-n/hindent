...
mkType GHC.HsQualTy {..} =
  ConstrainedType
    { context = mkContext <$> fromGenLocated hst_ctxt
...
mkTypeArguments ::
  ...
  -> [TypeArgument]
mkTypeArguments [] = []
mkTypeArguments (typeArg:rest) =
  case mkTypeArgument typeArg of
    Just arg -> arg : mkTypeArguments rest
    Nothing -> mkTypeArguments rest
#if MIN_VERSION_ghc_lib_parser(9, 14, 0)
